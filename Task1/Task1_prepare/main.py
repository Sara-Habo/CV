...
"background-color: rgb(255, 255, 255);")
    ...
    def CheckComboBox(self):
        ...
        if self.choosefilter.currentText() == "Choose filter":
            ...
            print("A")
            self.ShowFilteredImage(self.sharp_images)
            self.ShowFilteredInFrequency(self.sharp_images, 0)

    def High_filter_frequency(self, imagePath, flag):

        self.image = plt.imread(imagePath)
        if flag == 1: ##that mean if flag ==1  image is rgb and need to convert to gary to get frequency domain
            self.image = (cv2.cvtColor(self.image, cv2.COLOR_RGB2GRAY))

        #  Fourier transform
        fimg = np.fft.fft2(self.image)
